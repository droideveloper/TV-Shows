...
class LoadMoreTVShowIntent(
  private val page: Int,
  private val tvShowRepository: TVShowRepository): ObservableIntent<TVShowsModel>() {

  override fun invoke(): Observable<Reducer<TVShowsModel>> = tvShowRepository.shows(page = page)
    .concatMap(::success)
  ...
  private fun success(resource: Resource<List<TVShowEntity>>): Observable<Reducer<TVShowsModel>> = when(resource) {
    is Resource.Success<List<TVShowEntity>> -> Observable.just(
      { o -> o.copy(state = Operation(LOAD_MORE), data = resource.data ?: emptyList(), page = resource.page ?: 0, totalPage = resource.totalPage ?: 0) },
  ...
  private fun initial(): Reducer<TVShowsModel> = { o -> o.copy(state = Operation(LOAD_MORE), data = emptyList()) }
}